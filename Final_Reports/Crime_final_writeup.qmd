...
To prepare this EMS and Crime location map, we had to use the `sp` and `tmap` package instead of `ggplot2` and `plotly`. We were most familiar with latter packages but the `sp` package included the function, `read-sf()`, that was able to use gis imaging to recognize the ward data in the `.shp`. Since the ward does not have discrete longitude and latitude data, this ability provided by the `sp` was vital. The `tmap` worked best with the `sp`, so once we read in the data, it was converted into a map using the `tm_shape()` function in `tmap`. Even though we could use `ggplot()` to plot the longitude and latitude coordinates, we had to use the `st_as_sf()` to plot the EMS and VIOLENT_CRIME points. Afterwards, we could add (+) the three individual maps into one so they can overlap the points and ward boundaries.


## General Linear Model (GLM)

Another statistical model we attempted to run was the general linear model, GLM. The “VIOLENT_CRIME” was the binary variable, predicted outcome, and the “report_start_diff” was a vector of predictor variables. The “report_start_diff” was the time difference between our report and start date. We hope to see that the greater the time difference between the report and start date was, it was most likely to be a nonviolent crime. 
...
On the other hand, our visualization did not portray such confidence. It depicted a negative exponential slope, yet the traditional S-shape curve did not form. When looking more into why our data did not perform as expected; some possibilities could be the unreliable reporting, the proportions of the nonviolent and violent crimes. Our data was majority nonviolent crime (84% vs 16%) possibly resulting in our model to over fit. Also, we had to filter out negative time differences we had calculated which shows evidence of inaccuracy between the recording process causing the misshape in the GLM visual. Regardless of the errors, from the measures of central tendency, it appears that violent crime is most likely to be reported faster than a nonviolent crime. Violent crime (0) had a smaller mean of 55.8 hours compared to the nonviolent (0), 77.1 hours. The median was less impacted by the outliers and supported the following trend: violent crime (0), 1.50 hours, and nonviolent (1), 5.70 hours. 
